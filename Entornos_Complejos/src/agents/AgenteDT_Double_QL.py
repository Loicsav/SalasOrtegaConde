...
class AgenteDT_Double_QL(Agent):
    def __init__(self, env, seed: int, num_episodes: int = 1000, discount_factor: float = 1.0, epsilon: float = 0.1, decay: bool = False, decay_rate:float=1000.0):
        ...
        self.Q2 = np.zeros([env.observation_space.n, self.nA])
    ...
    def get_action(self, state, n:int):
        ...
        best_actions = np.argwhere(self.Q1[state] + self.Q2[state] == np.max(self.Q1[state] + self.Q2[state])).flatten()  # Puede haber múltiples acciones con el mismo valor máximo
        best_action = np.random.choice(best_actions)  # Elegimos aleatoriamente entre las
        #best_action = np.argmax(self.Q1[state] + self.Q2[state])  # Seleccionamos la acción con el valor Q más alto sumando ambas tablas Q
    ...
    def update(self, state, action, reward, next_state, next_action, episode_state:int):
        ...
        if np.random.rand() < 0.5:
            ...
            # Actualizamos Q1 utilizando la acción seleccionada por Q2
            target = reward + self.discount_factor * self.Q2[next_state][best_action]
            ...
            # Actualizamos Q2 utilizando la acción seleccionada por Q1
            best_action = np.argmax(self.Q2[next_state])  # Obtenemos la acción para el siguiente estado utilizando la política epsilon-greedy
